...
import wvs.scanners.sqli.error_based  # noqa: F401
import wvs.scanners.sqli.time_based  # noqa: F401
import wvs.scanners.xss.reflected  # noqa: F401
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    scan_parser.add_argument(
        ...
        action="store_true",
        help="Confirm you have legal permission to scan the target",
    )
    scan_parser.add_argument(
        "--modules", nargs="+", help="Specific scanner modules to run (default: all)"
    )
    scan_parser.add_argument(
        "--threads",
        type=int,
    ...
    scan_parser.add_argument(
        "--wordlist", type=Path, help="Wordlist for directory brute-forcing"
    )
    scan_parser.add_argument(
        "--output",
        type=Path,
        default=Path("wvs_report.html"),
        help="Path to save the report",
    )
    scan_parser.add_argument(
        "--format", choices=["markdown", "html"], default="html", help="Report format"
    )

    # Crawl-only command
    crawl_parser = subparsers.add_parser(
        "crawl-only", help="Crawl the target and discover endpoints without scanning"
    )
    crawl_parser.add_argument("target", help="Target URL (e.g. http://example.com/)")
    crawl_parser.add_argument(
        "--i-have-authorization",
        action="store_true",
        help="Confirm you have legal permission to crawl the target",
    )
    crawl_parser.add_argument(
    ...
    crawl_parser.add_argument(
        "--output", type=Path, help="Path to save the discovered endpoints list"
    )
...
def run_scan(args) -> int:
    ...
    session = WvsSession(
        "WVS", rate_limit_per_second=rate_limit or 10, timeout_seconds=timeout
    )
    ...
    console.print(
        f"[yellow]Phase 2: Scanning endpoints with {len(scanners_to_run)} module(s)...[/yellow]"
    )
