...
def build_candidate_urls(
    base_url: str, words: list[str], extensions: list[str]
) -> list[str]:
...
class DirectoryFuzzer:
    def __init__(
        self,
        session: WvsSession,
        ignore_codes: set[int] = frozenset({404}),
        threads: int = 10,
    ):
    ...
    def _measure_baseline_404(self, base_url: str) -> None:
        ...
        test_url = urljoin(
            base_url if base_url.endswith("/") else base_url + "/", bogus_path
        )
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                logger.debug(
                    f"Measured soft-404 baseline size: {self.baseline_404_size} bytes"
                )
    ...
    def fuzz(
        self, base_url: str, words: list[str], extensions: list[str]
    ) -> list[Finding]:
        ...
        with ThreadPoolExecutor(max_workers=self.threads) as executor:
            future_to_url = {
                executor.submit(check_url, url): url for url in candidate_urls
            }
